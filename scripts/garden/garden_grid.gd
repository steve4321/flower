...
func _on_plot_clicked(index: int) -> void:
	...
	if plant == null:
		...
		# 开花 → 显示操作菜单（浇水无意义，但可以查看/移除）
		_show_flowering_options(index)
	else:
...
func _show_flowering_options(index: int) -> void:
	# 简单实现：左键浇水（无效果），右键移除
	# TODO: P3 加入培育选项
	var plant: Plant = GameState.get_plant(index)
	if plant == null:
		return
	# 暂时只在info显示信息
	info_label.text = "%s (%s) - 开花 ✅ | 右键移除" % [plant.display_name, plant.get_stage_name()]


func _input(event: InputEvent) -> void:
	# 种子菜单打开时忽略右键操作
	if seed_menu.visible:
		return
	# 右键移除植物
